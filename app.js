...
function fixSpacingRules(text) {
  ...
  //    (을/를/는/은/의/에서/에게/까지/부터/처럼/만큼/이나/보다 는 다른 뜻으로 잘 안 쓰여서 안전함)
  //    단, 바로 앞 글자가 이미 조사(가/이/을/를/은/는/의)라면 그건 앞 단어에 이미 붙어 끝난 것이고
  //    뒤에 오는 건 완전히 새로운 단어일 뿐이므로(예: "사람이 가장" → 절대 붙이면 안 됨) 건드리지 않는다.
  const PARTICLE_CHARS = new Set(['가', '이', '을', '를', '은', '는', '의']);
  text = text.replace(/([가-힣]) (을|를|는|은|의|에서|에게|까지|부터|처럼|만큼|이나|보다)/g, (m, prev, particle) => {
