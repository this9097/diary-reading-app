...
function wireEvents() {
  ...
  async function fetchNaverSpellcheck(text) {
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), 9000);
    try {
      const res = await fetch('/api/spellcheck?text=' + encodeURIComponent(text), { signal: controller.signal });
      clearTimeout(timer);
      ...
      clearTimeout(timer);
      return null; // 네트워크 오류/타임아웃/차단 등 — 아래에서 자체 규칙 검사로 넘어감
    }
  }
  ...
  $('#spellcheckBtn').addEventListener('click', async () => {
    ...
    // 1) 먼저 네이버 맞춤법 검사(서버 경유)를 시도 — 실패하면 자체 규칙 기반으로 자동 전환
    let text = null, source = null;
    const naverResult = await fetchNaverSpellcheck(before);
    if (naverResult && naverResult.trim() && naverResult !== before) {
      text = naverResult;
      source = 'naver';
    } else {
    ...
    toast(`${count}곳 고쳤습니다${source === 'naver' ? ' (네이버 맞춤법 검사)' : ''}`);
  });
