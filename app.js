...
/* =====================================================================
   ...
   시트부터 하나씩 닫도록 하는 안전한 히스토리 스택.

   핵심 원칙: history.back()을 부르는 곳은 딱 한 곳(consumeBackGuard)뿐이고,
   그 호출로 생기는 popstate는 suppressNextPopstate로 스스로 무시한다.
   그래서 "우리가 스스로 부른 back"과 "사용자가 진짜로 누른 뒤로가기"가
   절대 헷갈리지 않고, pushState/back 횟수가 항상 정확히 맞는다.
   ===================================================================== */
const overlayOpenStack = []; // 열린 순서대로 오버레이 id를 쌓아둠 (예: 'editorOverlay')
let suppressNextPopstate = 0;

function pushBackGuard(overlayId) {
  overlayOpenStack.push(overlayId);
  history.pushState({ appOverlay: overlayId, depth: overlayOpenStack.length }, '');
}
function consumeBackGuard(overlayId) {
  const idx = overlayOpenStack.lastIndexOf(overlayId);
  if (idx === -1) return; // 짝이 안 맞으면(이미 닫혔거나 애초에 push 안 됐으면) 아무 것도 안 함 — 안전
  overlayOpenStack.splice(idx, 1);
  suppressNextPopstate++;
  history.back();
}
window.addEventListener('popstate', () => {
  if (suppressNextPopstate > 0) { suppressNextPopstate--; return; } // 우리가 스스로 유발한 back → 이미 정리 끝, 무시
  const id = overlayOpenStack.pop(); // 가장 최근에 연 시트부터 닫는다
  if (!id) return; // 열린 시트가 없으면 진짜 뒤로가기(앱 종료 등)를 그대로 둔다
  if (id === 'bookFilter') { state.bookFilter = null; renderAll(); return; }
  const el = document.getElementById(id);
  if (el) el.classList.remove('show');
  if (id === 'editorOverlay') state.editingId = null;
});
...
function openEditor(entry) {
  ...
  pushBackGuard('editorOverlay');
  // 수정하는 경우(제목은 이미 채워져 있음)는 바로 본문으로, 새로 쓰는 경우는 제목부터
  if (entry) {
    setTimeout(() => { const b = $('#f_body'); b.focus(); b.setSelectionRange(b.value.length, b.value.length); }, 50);
  } else {
...
function closeEditor() {
  ...
  consumeBackGuard('editorOverlay');
}
...
// 한글 음절의 받침(종성)을 계산한다. 완성형 한글이 아니면 -1.
// 유니코드 한글 음절 = (초성*21 + 중성)*28 + 종성 + 0xAC00. 종성 인덱스 0=받침없음.
function hangulFinal(ch) {
  if (!ch) return -1;
  const code = ch.charCodeAt(0) - 0xAC00;
  if (code < 0 || code > 11171) return -1;
  return code % 28;
}
const FINAL_NONE = 0, FINAL_L = 8, FINAL_N = 4; // ㄹ받침, ㄴ받침

// 문법적으로 확실한 띄어쓰기 규칙 3가지 — 받침을 직접 분석해서 판단하므로
// 특정 단어 사전에 의존하지 않고 어떤 동사에도 두루 적용된다.
function fixSpacingRules(text) {
  let count = 0;

  // 1) "~ㄹ/을 + 수 + 있/없" → "~ㄹ 수 있/없다" (예: 할수있다 → 할 수 있다, 갈수없다 → 갈 수 없다)
  text = text.replace(/([가-힣])수(있|없)/g, (m, prev, next) => {
    if (hangulFinal(prev) === FINAL_L) { count++; return `${prev} 수 ${next}`; }
    return m;
  });

  // 2) "안" + 되/돼/된/됨/됩 (부정 표현) → 띄어쓰기 (예: 안되다 → 안 되다, 안돼요 → 안 돼요)
  //    "안됐다"(안쓰럽다는 뜻의 굳어진 표현)는 건드리지 않는다.
  text = text.replace(/안(되|돼|된|됨|됩)/g, (m, next) => { count++; return `안 ${next}`; });

  // 3) 관형형(는/ㄴ·은/ㄹ·을/던) 뒤에 오는 의존명사 "것" → 띄어쓰기
  //    (예: 하는것 → 하는 것, 먹은것 → 먹은 것, 할것 → 할 것, 하던것 → 하던 것)
  text = text.replace(/([가-힣])것/g, (m, prev) => {
    const f = hangulFinal(prev);
    if (f === FINAL_L || f === FINAL_N || prev === '는' || prev === '던') { count++; return `${prev} 것`; }
    return m;
  });

  // 4) 조사가 명사와 떨어져 있는 경우 → 붙여쓰기 (예: "외모 가" → "외모가", "마음 은" → "마음은")
  //    조사는 항상 앞말에 붙여 쓰는 게 원칙이라, 아래 조사들 앞에 공백이 있으면 제거한다.
  //    (을/를/는/은/의/도/에서/에게/까지/부터/처럼/만큼/이나/보다 는 다른 뜻으로 잘 안 쓰여서 안전함)
  //    단, 바로 앞 글자가 이미 조사(가/이/을/를/은/는/의/도)라면 그건 앞 단어에 이미 붙어 끝난 것이고
  //    뒤에 오는 건 완전히 새로운 단어일 뿐이므로(예: "사람이 가장" → 절대 붙이면 안 됨) 건드리지 않는다.
  //    ★ 매우 중요: 조사 바로 뒤에 한글이 더 이어지면(공백/문장부호/끝이 아니면) 절대 건드리지 않는다.
  //    "도서관"의 "도", "이유"의 "이", "은행"의 "은"처럼 조사가 아니라 더 긴 단어의 첫 글자일 수 있기 때문
  //    — 이걸 놓쳐서 실제로 "정원에 앉아 도서관" → "앉아도 서관"으로 단어가 깨지는 사고가 있었다.
  const PARTICLE_CHARS = new Set(['가', '이', '을', '를', '은', '는', '의', '도']);
  text = text.replace(/([가-힣]) (을|를|는|은|의|도|에서|에게|까지|부터|처럼|만큼|이나|보다)(?![가-힣])/g, (m, prev, particle) => {
    if (PARTICLE_CHARS.has(prev)) return m;
    count++;
    return `${prev}${particle}`;
  });

  // "가"는 받침 없는 명사 뒤에서만, "이"는 받침 있는 명사 뒤에서만 조사로 붙는다.
  // 단, "~에 가다/가서/가자"처럼 진짜 '가다' 동사인 경우와, 앞 글자가 이미 조사인 경우는 건드리지 않는다.
  // ★ 이 역시 뒤에 한글이 더 있으면 건드리지 않는다("가족"의 "가", "이번"의 "이" 등과 헷갈리지 않도록).
  //   (이 조건 때문에 "학생 이다" → "학생이다" 같은 경우는 더 이상 자동으로 못 잡는다 — 안전이 우선)
  text = text.replace(/([가-힣]) (가|이)(?![가-힣])/g, (m, prev, particle, offset, full) => {
    if (PARTICLE_CHARS.has(prev)) return m;
    const f = hangulFinal(prev);
    if (particle === '가' && f !== FINAL_NONE) return m;
    if (particle === '이' && f === FINAL_NONE) return m;
    if (particle === '가') {
      const twoBefore = full.slice(Math.max(0, offset - 1), offset + 1); // prev 포함 2글자(으로/까지/부터/에서 판별용)
      if (prev === '에' || prev === '로' || ['으로', '까지', '부터', '에서'].includes(twoBefore)) return m;
    }
    count++;
    return `${prev}${particle}`;
  });

  // 5) 반대로 "을/를" 뒤에 공백 없이 바로 다음 말이 붙어버린 경우 → 띄어쓰기
  //    (예: "밥을먹었다" → "밥을 먹었다", "학교를갔다" → "학교를 갔다")
  //    을/를로 시작하는 한글 단어는 사실상 없기 때문에 안전하게 적용 가능하다.
  //    ※ "이/가"는 이걸로 시작하는 단어가 아주 많아서(가족, 가끔, 이유...) 여기서는 다루지 않는다.
  text = text.replace(/(을|를)([가-힣])/g, (m, particle, next) => {
    count++;
    return `${particle} ${next}`;
  });

  // 6) "것" 바로 뒤에 "같다"가 붙어있는 경우 → 띄어쓰기 (예: "할것같다" → "할 것 같다"의 나머지 절반)
  text = text.replace(/것(같)/g, (m, next) => { count++; return `것 ${next}`; });
...
function runSpellCheck(text) {
  ...
  // 가장 중요하게 다뤄야 할 부분: 띄어쓰기 규칙부터 먼저 적용
  const spacingResult = fixSpacingRules(result);
...
function openSettings() {
  ...
  pushBackGuard('settingsOverlay');
}
function closeSettings() {
  $('#settingsOverlay').classList.remove('show');
  consumeBackGuard('settingsOverlay');
}
...
function openPhotoImport() {
  ...
  pushBackGuard('photoImportOverlay');
}
function closePhotoImport() {
  $('#photoImportOverlay').classList.remove('show');
  consumeBackGuard('photoImportOverlay');
}
...
function clearBookFilter() {
  if (state.bookFilter) { state.bookFilter = null; consumeBackGuard('bookFilter'); }
}
function wireEvents() {
  $$('.ribbon').forEach(r => r.addEventListener('click', () => {
    state.activeTab = r.dataset.tab; state.search = ''; $('#searchInput').value = ''; clearBookFilter(); renderAll();
  }));
  $('#searchInput').addEventListener('input', (e) => { state.search = e.target.value; clearBookFilter(); renderAll(); });
  $('#searchClear').addEventListener('click', () => { state.search = ''; $('#searchInput').value = ''; $('#searchInput').focus(); renderAll(); });
  $('#bfb_clear').addEventListener('click', () => { clearBookFilter(); renderAll(); });
  ...
  function countMarks(html) { return (html.match(/<mark>/g) || []).length; }

  async function fetchJsonSpellcheck(endpoint, text, timeoutMs) {
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), timeoutMs);
    try {
      const res = await fetch(endpoint + '?text=' + encodeURIComponent(text), { signal: controller.signal });
      clearTimeout(timer);
      if (!res.ok) return null;
      const data = await res.json();
      if (!data || typeof data.correctedText !== 'string' || !data.correctedText.trim()) return null;
      return data.correctedText;
    } catch (e) {
      clearTimeout(timer);
      return null; // 네트워크 오류/타임아웃/차단/키 미설정 등 — 다음 단계로 넘어감
    }
  }
  ...
  $('#spellcheckBtn').addEventListener('click', async () => {
    ...
    // 1) Claude(AI) 교정 시도 → 2) 실패 시 네이버 맞춤법 검사 → 3) 그마저 실패하면 자체 규칙 검사
    let text = null, source = null;
    const aiResult = await fetchJsonSpellcheck('/api/ai-spellcheck', before, 15000);
    if (aiResult && aiResult !== before) {
      text = aiResult;
      source = 'ai';
    } else {
      const naverResult = await fetchJsonSpellcheck('/api/spellcheck', before, 9000);
      if (naverResult && naverResult !== before) {
        text = naverResult;
        source = 'naver';
      } else {
        const r = runSpellCheck(before);
        if (r.count > 0) { text = r.text; source = 'local'; }
      }
    }
    ...
    if (text === null) { toast('고칠 부분을 찾지 못했습니다'); return; }
    const previewHtml = buildSpellcheckPreviewHtml(before, text);
    const count = countMarks(previewHtml);
    if (count === 0) { toast('고칠 부분을 찾지 못했습니다'); return; }

    spellcheckOriginalText = before;
    $('#f_body').value = text; // 바로 적용
    $('#spellcheckPreview').innerHTML = previewHtml;
    $('#spellcheckPreviewField').style.display = 'block';
    const sourceLabel = source === 'ai' ? ' (AI 교정)' : source === 'naver' ? ' (네이버 맞춤법 검사)' : '';
    toast(`${count}곳 고쳤습니다${sourceLabel}`);
  });
