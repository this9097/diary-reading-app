...
function wireEvents() {
  ...
  // 예전에 적었던 책 제목/저자를 눌러서 고를 수 있는 목록. 제목을 목록에서 그대로
  // 고르면 그 책의 저자도 자동으로 채워준다 (저자 칸에 이미 값이 있으면 덮어쓰지 않음).
  setupAutocomplete('#f_title', '#titleAcList', () => uniqueSorted(e => e.title), (title) => {
    if ($('#f_author').value.trim()) return;
