...
function fixSpacingRules(text) {
  ...
  text = text.replace(/([가-힣]) (을|를|는|은|의|도|에서|에게|까지|부터|처럼|만큼|이나|보다)/g, (m, prev, particle) => {
    if (PARTICLE_CHARS.has(prev)) return m;
    count++;
    return `${prev}${particle}`;
  });

  // "가"는 받침 없는 명사 뒤에서만, "이"는 받침 있는 명사 뒤에서만 조사(또는 "이다"의 "이")로 붙는다.
  // 단, "~에 가다/가서/가자"처럼 진짜 '가다' 동사인 경우와, 앞 글자가 이미 조사인 경우는 건드리지 않는다.
  text = text.replace(/([가-힣]) (가|이)/g, (m, prev, particle, offset, full) => {
    if (PARTICLE_CHARS.has(prev)) return m;
    const f = hangulFinal(prev);
